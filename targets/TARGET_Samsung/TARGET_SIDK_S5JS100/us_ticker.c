...
uint32_t us_ticker_read()
{
    ...
    g_us_last_return = TIMER_TARGET_COUNT_DEFAULT-current_count;
    return TIMER_TARGET_COUNT_DEFAULT-current_count;
}
...
void us_ticker_set_interrupt(timestamp_t timestamp)
{
    if (timestamp < 0x70000000 && timestamp != 0) {
        ...
        us_user_intset = us_user_intset;
        uint32_t past_tick = us_ticker_read() - temp;
        putreg32(0, S5JS100_TIMER1_BASE + S5JS100_TIMER_LOAD_CON_VALUE);
        putreg32(us_user_intset - past_tick, S5JS100_TIMER1_BASE + S5JS100_TIMER_INT_SEL);
