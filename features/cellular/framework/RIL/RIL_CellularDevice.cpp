...
{
    _api_mutex.lock();
    nsapi_error_t ret = lock_and_send_request(RIL_REQUEST_GET_SIM_STATUS, NULL, 0, callback(this, &RIL_CellularDevice::sim_status_response));
                                                        ...
                                                        int timeout)
{
    ...
    if (token) {
        bool time_out = token->cond_var->wait_for(timeout);
        if (time_out) {
